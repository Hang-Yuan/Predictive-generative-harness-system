...
凡步骤涉及写入流水/记录文件（MEMORY_LOG / ITERATION_LOG / LTM / _本周.md / 候选池 procedural_memory/semantic_memory），**写入前必须先 Read 目标文件**，识别其 §条目模板 / §机制要点 / §条目格式 节，然后**严格按格式追加**，不偏离不附加：
